...
public class Problem188 {
    public int maxProfit(int k, int[] prices) {
        ...
        for(int i = 0; i < k && changed; i++) {
            ...
            newPrices[0] = prices[0];
            for(int j = 1; j < prices.length; j++) {
                newPrices[j] = prices[j] - profits[j - 1];
            }
            int min = newPrices[0], max = prices[0];
            for(int j = 1; j < prices.length; j++) {
                if(newPrices[j] < min) {
                    min = newPrices[j];
                    max = prices[j];
                ...
                int profit = Math.max(profits[j - 1], max - min);
                if (profit > profits[j]) {
                    profits[j] = profit;
                    result = Math.max(result, profits[j]);
                    changed = true;
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(new Problem188().maxProfit(100, new int[]{
